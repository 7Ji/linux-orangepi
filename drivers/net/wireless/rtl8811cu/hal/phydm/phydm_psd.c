...
u32 phydm_get_psd_data(void *dm_void, u32 psd_tone_idx, u32 igi)
{
	...
	if (dm->support_ic_type & ODM_RTL8821C) {
		psd_report = odm_get_bb_reg(dm, dm_psd_table->psd_report_reg, 0xffffff);
...
void phydm_psd(void *dm_void, u32 igi, u16 start_point, u16 stop_point)
{
	...
	/*[Backup RF Reg]*/
	dm_psd_table->rf_0x18_bkp = odm_get_rf_reg(dm, RF_PATH_A, RF_0x18, RFREGOFFSETMASK);
	dm_psd_table->rf_0x18_bkp_b = odm_get_rf_reg(dm, RF_PATH_B, RF_0x18, RFREGOFFSETMASK);
	...
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x18, 0xc00, dm_psd_table->psd_bw_rf_reg); /*2b'11: 20MHz, 2b'10: 40MHz, 2b'01: 80MHz */
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x18, 0xf0000, ag_rf_mode_reg); /* Set RF ag fc mode*/

	/* RF path-b */
	odm_set_rf_reg(dm, RF_PATH_B, RF_0x18, 0xff, psd_fc_channel); /* Set RF fc*/
	odm_set_rf_reg(dm, RF_PATH_B, RF_0x18, 0x300, rf_reg18_9_8);
	odm_set_rf_reg(dm, RF_PATH_B, RF_0x18, 0xc00, dm_psd_table->psd_bw_rf_reg); /*2b'11: 20MHz, 2b'10: 40MHz, 2b'01: 80MHz */
	...
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x18, RFREGOFFSETMASK, dm_psd_table->rf_0x18_bkp);
	odm_set_rf_reg(dm, RF_PATH_B, RF_0x18, RFREGOFFSETMASK, dm_psd_table->rf_0x18_bkp_b);
			    ...
			    u8 psd_input, u8 channel, u8 noise_k_en)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(dm, R_0x910, BIT(15) | BIT(14), fft_smp_point_idx);
