...
void halrf_iqk_xym_read(void *dm_void, u8 path,
			u8 xym_type
			/*0: rx_sym; 1: tx_xym; 2:gs1_xym; 3:gs2_sym; 4: rxk1_xym*/
			)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
...
void halrf_iqk_xym_show(struct dm_struct *dm,
			u8 xym_type
			/*0: rx_sym; 1: tx_xym; 2:gs1_xym; 3:gs2_sym; 4: rxk1_xym*/
			)
{
...
#if (RTL8814A_SUPPORT == 1 || RTL8822B_SUPPORT == 1 || RTL8821C_SUPPORT == 1 || RTL8195B_SUPPORT == 1)
	PDM_SNPF(out_len, used, output + used, out_len - used, "%-20s: %s\n",
...
void halrf_cmn_info_set(void *dm_void, u32 cmn_info, u64 value)
{
	...
	case HALRF_CMNINFO_MP_PSD_AVERAGE:
		...
		break;
#endif
	case HALRF_CMNINFO_MP_RATE_IDX:
		rf->mp_rate_idx = (u8)value;
		break;
	default:
		/* do nothing */
		break;
...
void halrf_watchdog(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
#if 0
	/*RF_DBG(dm, DBG_RF_TMP, "halrf_watchdog\n");*/
#endif
